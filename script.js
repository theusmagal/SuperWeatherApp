...
const weatherApiKey = WEATHERAPI_KEY;
...
async function getWeather() {
  ...
  try {
    ...
    // 24h forecast comparison from WeatherAPI
...
async function getWeatherByLocation() {
  ...
  navigator.geolocation.getCurrentPosition(async position => {
    ...
    try {
      const openUrl = `https://api.openweathermap.org/data/3.0/onecall?lat=${lat}&lon=${lon}&exclude=minutely,alerts&appid=${OPENWEATHER_KEY}&units=${unit}`;
      ...
      const weatherUrl = `https://api.weatherapi.com/v1/forecast.json?key=${WEATHERAPI_KEY}&q=${lat},${lon}&days=1&aqi=no&alerts=no`;
